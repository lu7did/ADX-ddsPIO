...
A new, fresh, board design will eventually be made, however the development is supported with 
a prototype based on a modified RDX_rp2040 board.
...
This option is deactivated with the compilation directive #define SI4732 1
```
 
### Si4732 receiver

When the board uses a Si4732 based receiver the clock generated by the rp2040 is only needed
for transmission as the receiver generates it's own clock.

For simplicity the transmission clock (CLK0 and CLK1) are turned off while in reception and
turned on while on transmission.

```
This option is activated with the compilation directive #define SI4732 1
This option override all other configuration options
```
...
### RDX-rp2040 modified prototype

To continue the development of the firmware adding the Si4732 based receptor a modified
version of a board from the RDX-rp2040 project has been used with small mods in order
to adapt for the differences

...
int32_t pack28(const char* callsign)
{
    ...
    if (starts_with(callsign, "3DA0") && length <= 7)
    {
        ...
        memcpy(c6, "3D0", 3);
        //*fix* memcpy(c6 + 3, callsign + 4, length - 4);
        if (length > 4) {
           size_t n = (size_t)(length - 4);
           memcpy(c6 + 3, callsign + 4, n);
        }
    ...
    else if (starts_with(callsign, "3X") && is_letter(callsign[2]) && length <= 7)
    {
        ...
        memcpy(c6, "Q", 1);
        //*fix* memcpy(c6 + 1, callsign + 2, length - 2);
        
        if (length > 2) {
           size_t n = (size_t)(length - 2);
           memcpy(c6 + 1, callsign + 2, n);
        }
    ...
    else
    {
        ...
        {
            // AB0XYZ
            //*fix* memcpy(c6, callsign, length);
            if (length > 0)
               memcpy(c6, callsign, (size_t)length);

        }
        else if (is_digit(callsign[1]) && length <= 5)
        {
            // A0XYZ -> " A0XYZ"
            //*fix* memcpy(c6 + 1, callsign, length);
            if (length > 0)
               memcpy(c6 + 1, callsign, (size_t)length);
    ...
    if ((i0 = char_index(A1, c6[0])) >= 0 && (i1 = char_index(A2, c6[1])) >= 0 && (i2 = char_index(A3, c6[2])) >= 0 && (i3 = char_index(A4, c6[3])) >= 0 && (i4 = char_index(A4, c6[4])) >= 0 && (i5 = char_index(A4, c6[5])) >= 0)
    {
        ...
        n28 = n28 * 27 + i4;
        n28 = n28 * 27 + i5;
        //*fix* return NTOKENS + MAX22 + n28;
        return NTOKENS + MAX22 + (size_t)n28;
        //*fix* int32_t base = (int32_t)(NTOKENS + MAX22);   // OJO: ver nota abajo

        if (n28 < 0) return base;                        // o error
           uint64_t tmp = (uint64_t)base + (uint64_t)(uint32_t)n28;
        if (tmp > (uint64_t)INT32_MAX) return INT32_MAX; // o manejar error
           return (int32_t)tmp;

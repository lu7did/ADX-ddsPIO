...
void save_wav(const float* signal, int num_samples, int sample_rate, const char* path)
{
    ...
    //*fix* uint32_t sampleRate = sample_rate;
    if (sample_rate < 0) {
       return; // o manejar error
    }   
    uint32_t sampleRate = (uint32_t)sample_rate;

    //*fix* uint16_t blockAlign = numChannels * bitsPerSample / 8;
    uint32_t tmp = (uint32_t)numChannels * (uint32_t)bitsPerSample / 8;
    if (tmp > UINT16_MAX) {
    // manejar error (o truncar explícitamente si corresponde)
       tmp = UINT16_MAX;  
    }
    uint16_t blockAlign = (uint16_t)tmp;

    
    uint32_t byteRate = sampleRate * blockAlign;

    char subChunk2ID[4] = { 'd', 'a', 't', 'a' };
    //*fix* uint32_t subChunk2Size = num_samples * blockAlign;
    uint32_t subChunk2Size = (uint32_t)num_samples * (uint32_t)blockAlign;
    ...
    int16_t* raw_data = (int16_t*)malloc((size_t)num_samples * (size_t)blockAlign);
    ...
    for (int i = 0; i < num_samples; i++)
    {
        ...
        if (x > 1.0f)
            ...
            x = -1.0f;
        //*fix* raw_data[i] = (int)(0.5 + (x * 32767.0));
        raw_data[i] = (int)(0.5f + (x * 32767.0f));
